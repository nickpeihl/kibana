...
import type { ZodObjectType, ZodObject, ZodType, ZodOptional } from '@kbn/zod';
...
/** Explicit function signature for the drilldown registry's getSchema method. */
export type GetDrilldownsSchemaFnType = (supportedTriggers: string[]) => ZodObject<{
  drilldowns: ZodOptional<ZodType<DrilldownState[]>>;
}>;

export type DrilldownTransforms = {
  transformIn: <State extends SerializedDrilldowns>(
    state: State
  ) => { state: State; references: Reference[] };
  transformOut: <StoredState extends SerializedDrilldowns>(
