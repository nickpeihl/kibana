...
export const isEmbeddableEditorState = (state: unknown): state is EmbeddableEditorState => {
...
export const isEmbeddablePackageState = (state: unknown): state is EmbeddablePackageState => {
  return (
...
const ensureFieldOfTypeExists = (key: string, obj: unknown, type?: string): boolean => {
  return (
    Boolean(obj) &&
