...
const queriesOmittedWarningSchema = schema.object({
  type: schema.literal('queries_omitted'),
  message: schema.string(),
});

export const persesWarningsSchema = schema.arrayOf(
  schema.oneOf([droppedPanelWarningSchema, queriesOmittedWarningSchema])
);
